...
function initTheme() {
  ...
  let lightThemeSelected = localStorage.getItem("themeSwitch") !== null && localStorage.getItem("themeSwitch") === "light-mode";
...
async function getNutritionalData() {
  ...
  try {
    const res = await fetch("https://edamam-edamam-nutrition-analysis.p.rapidapi.com/api/nutrition-data?nutrition-type=logging&ingr=".concat(stringToArray(search.value)), {
      method: "GET",
      headers: {
        "x-rapidapi-host": "edamam-edamam-nutrition-analysis.p.rapidapi.com",
        "x-rapidapi-key": "14c27a0fdfmsh8d07d5fe4e0ad0fp14ecd7jsn68e1c239df59"
      }
    });
    ...
    let macroNutrientsGroup = [Math.round(nutritionalData.totalNutrients.CHOCDF.quantity), Math.round(nutritionalData.totalNutrients.PROCNT.quantity), Math.round(nutritionalData.totalNutrients.FAT.quantity)];
    let microNutrientsGroup = [Math.round(nutritionalData.totalNutrients.FE.quantity), Math.round(nutritionalData.totalNutrients.MG.quantity), Math.round(nutritionalData.totalNutrients.NA.quantity), Math.round(nutritionalData.totalNutrients.CA.quantity), Math.round(nutritionalData.totalNutrients.K.quantity), Math.round(nutritionalData.totalNutrients.CHOLE.quantity), Math.round(nutritionalData.totalNutrients.VITB6A.quantity), Math.round(nutritionalData.totalNutrients.VITC.quantity), Math.round(nutritionalData.totalNutrients.ZN.quantity)];
    /* code below for charts */

    /* Chart.defaults.global.defaultFontColor = "#999"; */

    let macroNutrientChart = document.getElementById("macro-nutrient-chart").getContext("2d");
    ...
    let microNutrientChart = document.getElementById("micro-bar-chart").getContext("2d");
    let microNutrientBarChart = new Chart(microNutrientChart, {
      type: "bar",
      data: {
        labels: ["Iron", "Magnesium", "Sodium", "Calcium", "Potassium", "Cholesterol", "Vitamin B6", "Vitamin C", "Zinc"],
        datasets: [{
    ...
    const totalCarbsQuantity = Math.round(nutritionalData.totalNutrients.CHOCDF.quantity);
    const totalProteinQuantity = Math.round(nutritionalData.totalNutrients.PROCNT.quantity);
    const totalFatQuantity = Math.round(nutritionalData.totalNutrients.FAT.quantity);
    const threeMacrosSum = totalCarbsQuantity + totalProteinQuantity + totalFatQuantity;
    ...
    alert("We're sorry we couldn't find that in our database. Either check your internet connection or try another search for a food with a measurement ex. 250g sweet potato");
...
displaySugarValue = sugarValue => {
  if (sugarValue === undefined) {
    document.getElementsByClassName("nutrient-value")[6].textContent = "0%";
  } else document.getElementsByClassName("nutrient-value")[6].textContent = "".concat(getPercentage(sugarValue, 50), "%");
};
...
displayCaloriesAmount = calorieData => {
  const calorieAmount = document.getElementsByClassName("calorie-amount")[0];
  calorieAmount.textContent = calorieData;
};

displayMeasurementAmount = weightData => {
  const measurementAmount = document.getElementsByClassName("measurement-amount")[0];
  measurementAmount.textContent = "".concat(weightData, " g");
};

displayCaloriesFromCarbs = (carbsValue, totalMacrosValue, totalCalories) => {
  let caloriesFromCarbsNumber = document.getElementById("calories-from-carbs-number");
  let carbsPercentage = carbsValue / totalMacrosValue;
  caloriesFromCarbsNumber.textContent = Math.round(carbsPercentage * totalCalories);
};

displayCaloriesFromProtein = (proteinValue, totalMacrosValue, totalCalories) => {
  let caloriesFromProteinNumber = document.getElementById("calories-from-protein-number");
  let proteinPercentage = proteinValue / totalMacrosValue;
  caloriesFromProteinNumber.textContent = Math.round(proteinPercentage * totalCalories);
};

displayCaloriesFromFat = (fatValue, totalMacrosValue, totalCalories) => {
  let caloriesFromFatNumber = document.getElementById("calories-from-fat-number");
...
displayNutrientDailyValues = (nutrientValue, nutrientText) => {
  if (nutrientValue === undefined) return;
  nutrientText.textContent = "".concat(Math.round(nutrientValue), "%");
};
/* get value to display daily sugar value */


getPercentage = (smallNumber, largerNumber) => {
